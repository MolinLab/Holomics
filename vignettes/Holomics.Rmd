...
#### Omics dataset
Omics datasets can contain molecular features measured on a continuous scale (e.g., microarray, mass spectrometry-based metabolomics) or sequence-based count data (e.g., RNA-seq, 16S rRNA amplicon sequencing), which become continuous after pre-processing and normalization. In general, the omics dataset must be a numeric matrix (essentially a count table) in either .xlsx, .csv or .txt format, where rows represent samples and columns represent measured features (see Table \@ref(tab:omicsData)).
The first column has to contain the sample names and the first row the feature names. There are no strict restrictions on the characters and symbols used in names, but it is recommended to minimize the use of special characters. It is important that all omics datasets used together in an analysis share the same sample names and follow the same sample order. If the dataset contains more features (columns) than Excel allows, the matrix can also be uploaded in transposed format.
...
To upload an omics dataset (e.g., a transcriptomics read count table or a metabolomics table), the "Omics data" data type should be selected. Then, the corresponding .xlsx,.csv or .txt file must be chosen via the file upload, and the separator that separator the data entries needs to be chosen. If the uploaded file is a .xlsx the separator field can be ignored.

Afterwards a file name must be entered, which will later be used on the analysis pages to easily identify the dataset.

Next, you must indicate whether the dataset has already been filtered at an earlier stage (preferably using Holomics) by selecting the "Was previously filtered" checkbox. If the dataset originates from microbial community analysis (e.g., an OTU/ASV table), be sure to check the "Contains microbiome data" checkbox. This triggers a necessary preprocessing step—namely, the mixMC pipeline for microbiome data [@le2016mixmc].

If the dataset is in transposed format (i.e., samples in columns and features in rows), activate the "Has transposed format" checkbox.

Afterward, the type of analysis for the dataset(s) must be selected. Datasets can only be used for multi-omics analysis if the "Was previously filtered" checkbox is selected (see also the note above). Finally, you can choose or manually enter the plot name to be used for the dataset in visualizations. Note that this plot name does not need to match the earlier file name and can even be the same for multiple datasets.
...
To upload the metadata file containing the labels or class information for each sample, the "Labels/Classes" data type must be selected before uploading the corresponding .xlsx or .csv file. Additionally, a name for the metadata file must be provided.

If the metadata file includes an optional third column with color codes (either HEX codes or ASCII color names) for each sample, the "Includes color code" checkbox should be selected. This custom color scheme will then be applied in most of the plots to visually distinguish the different classes.
